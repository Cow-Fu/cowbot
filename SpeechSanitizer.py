...
class SpeechSanitizer:
    ...
    # TODO change to take the message itself, then can check for attachments and convert them to text
    ...
    def __init__(self, client: commands.Bot):
        self.regex_emoji = re.compile(r"<:(\w+):(\d+)>")
        self.regex_user = re.compile(r"<@(\d+)>")
        self.regex_channel = re.compile(r"<#(\d+)>")
        self.regex_url = re.compile(r"http\S+")
        self.client = client
